...
class DDPG():
    def __init__(self, params, use_wandb=False):
        if params['gym_environment'] != 'TB3':
            ...
            self.env = UnitySafetyGym(editor_run=False, env_type="windows", worker_id=int(time.time())%10000,
                                      time_scale=100, no_graphics=True, max_step=100,
                                      action_space_type="continuous", cost_penalty=True)
    ...
    def get_tanh_action(self, state, target=False) -> torch.Tensor:
        """
        Get the action from the actor model and apply the tanh activation function.
        Due the fact that the model class cannot be modified, we need to create this method to apply the tanh activation function
        
        Args:
            state (torch.Tensor): the current state
            target (bool): if the target model should be used or not

        Returns:
            tanh_actions (torch.Tensor): the actions with the tanh activation function applied
        """
        if target:
    ...
    def update_policy(self, memory_buffer: list) -> None:
        ...
        # Formula 5 from the DDPG paper
        # applied as Algorithm 1 from the DDPG paper
