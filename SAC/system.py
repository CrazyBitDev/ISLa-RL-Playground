...
class System:
    ...
    def initialization(self):
        ...
        for init_step in range(0, self.init_steps):            
            ...
            next_state, reward, _, _, _ = self.env.step(scale_action(action, self.min_action, self.max_action))
            #if self.original_state:
    ...
    def interaction(self, learn=True, remember=True):   
        ...
        for env_step in range(0, self.env_steps):
            ...
            cuda_state = torch.FloatTensor(self.state).unsqueeze(0).to(device)         
